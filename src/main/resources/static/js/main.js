    ...
    mainSearchForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (!API_ENABLED) {
                ...
                trains = [
                    { id: 1, trainNumber: 'TR001', trainName: 'Hanoi Express', source, destination, departureTime: new Date().toISOString(), arrivalTime: new Date(Date.now()+1000*60*60*6).toISOString(), fare: 150000, trainType: 'Express', availableSeats: 20 },
                    { id: 2, trainNumber: 'TR002', trainName: 'Coastal Line', source, destination, departureTime: new Date().toISOString(), arrivalTime: new Date(Date.now()+1000*60*60*8).toISOString(), fare: 120000, trainType: 'Regional', availableSeats: 5 }
                ];
            } else {
            ...
            sessionStorage.setItem('searchParams', JSON.stringify({ source, destination, date }));
    ...
    resultsSection.innerHTML = `
        <div class="container">
            <div class="results-header">
                ...
                <p>${trains.length} train(s) found from ${params.source} to ${params.destination} on ${params.date}</p>
            </div>
                        ...
                        <div class="train-info">
                            ...
                            <div class="route-info">
                                <div class="station">
                                    ...
                                    <span class="time">${new Date(train.departureTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</span>
                                </div>
                                <div class="route-arrow">→</div>
                                <div class="station">
                                    <strong>${train.destination}</strong>
                                    <span class="time">${new Date(train.arrivalTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</span>
                                </div>
                            </div>
                            <div class="train-details">
                                <span>Type: ${train.trainType}</span>
                                <span>Available Seats: ${train.availableSeats}</span>
                                <span class="fare">₫${train.fare.toLocaleString()}</span>
                            </div>
                        </div>
                        <div class="train-actions">
                            <button onclick="bookTrain(${train.id})" class="btn-book">Book Now</button>
                        </div>
                    </div>
                `).join('')}
            </div>
...
async function bookTrain(trainId) {
    ...
    // Store train ID and redirect to booking page
    sessionStorage.setItem('selectedTrainId', trainId);
    window.location.href = 'bookings.html?book=' + trainId;
}
